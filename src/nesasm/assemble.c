...
void assemble(void) {
    ...
    if (oplook(&i) >= 0) {
      if (opflg == PSEUDO) {
        switch (opval) {
          ...
          case P_ENDIF:  // .endif
            if (!check_eol(&i)) return;
            if (if_state[if_level] && (pass == LAST_PASS)) println();
            skip_lines = !if_state[if_level];
            if_level--;
            if (if_level == 0) in_if = 0;
            return;
        }
      }
    }
  ...
  if ((j == 0) || ((i != SFIELD) && (c != ':')))
    ...
    if (colsym(&i) != 0)
      if ((lablptr = stlook(1)) == NULL) return;
    if ((lablptr) && (prlnbuf[i] == ':')) i++;
  }
  ...
  if (mptr) {
    ...
    if (pass == LAST_PASS) {
      if (!asm_opt[OPT_MACRO]) loadlc((page << 13) + loccnt, 0);
    }

    /* get macro args */
    if (!macro_getargs(ip)) return;
  ...
  if (flag < 0) {
    labldef(loccnt, 1);
    if ((flag == -1)) error("Unknown instruction!");
    if ((flag == -2) && (pass == LAST_PASS)) {
      if (lablptr) loadlc(loccnt, 0);
      println();
    }
    lastlabl = NULL;
    return;
...
void do_if(int *ip) {
  ...
  if (!skip_lines) skip_lines = if_flag[if_level] = value ? 0 : 1;
...
void do_else(int *ip) {
  if (!in_if) fatal_error("Unexpected ELSE!");
}

/* .endif pseudo */

void do_endif(int *ip) {
  if (!in_if) fatal_error("Unexpected ENDIF!");
}
