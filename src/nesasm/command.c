...
char pseudo_flag[] = {0x0C, 0x0C, 0x0F, 0x0F, 0x0F, 0x0C, 0x0C, 0x0C, 0x0F,
                      0x0C, 0x0C, 0x0C, 0x0C, 0x0F, 0x0F, 0x0F, 0x0F, 0x0F,
                      0x0F, 0x0F, 0x0F, 0x0F, 0x0F, 0x0F, 0x0F, 0x0F, 0x0F,
                      0x0F, 0x0F, 0x0C, 0x0F, 0x0F, 0x0F, 0x0C, 0x0C, 0x0C,
                      0x0C, 0x0F, 0x0F, 0x0F, 0x0F, 0x0F, 0x0C, 0x0C, 0x0C,
...
void do_pseudo(int *ip) {
  ...
  switch (opval) {
    ...
    case P_DW:
      if (lastlabl) {
        if (lastlabl->data_type != P_DB) lastlabl = NULL;
      }
      break;

    default:
      if (lastlabl) {
        if (lastlabl->data_type != opval) lastlabl = NULL;
      }
      break;
  }
  ...
  for (;;) {
    ...
    if (prlnbuf[*ip] == '\"') {
      for (;;) {
        ...
        if (pass == LAST_PASS) putbyte(loccnt, c);
...
void do_org(int *ip) {
  ...
  switch (section) {
    ...
    case S_BSS:
      /* ram section */
      if ((value < machine->ram_base) ||
          (value >= (machine->ram_base + machine->ram_limit))) {
        error("Invalid address!");
        return;
      }
      break;
...
void do_bank(int *ip) {
  ...
  switch (prlnbuf[*ip]) {
    ...
    case ',':
      /* get name */
      (*ip)++;
      if (!getstring(ip, name, 63)) return;
...
void do_incbin(int *ip) {
  ...
  /* get file name */
  if (!getstring(ip, fname, 127)) return;
  ...
  if ((p = strrchr(fname, '.')) != NULL) {
    if (!strchr(p, PATH_SEPARATOR)) {
      ...
      if (!strcasecmp(p, ".fmp")) {
        if (pce_load_map(fname, 0)) return;
      }
    }
  }
  ...
  /* output */
  if (pass == LAST_PASS) loadlc(loccnt, 0);
  ...
  if (lablptr) {
    ...
    if (lastlabl) {
      if (lastlabl->data_type == P_INCBIN) lastlabl->data_size += size;
    }
  }
}
...
void do_mx(char *fname) {
  ...
  while (fgets(line, 254, fp) != NULL) {
    if (line[0] == 'S') {
      ...
      /* checksum */
      chksum = cnt + ((addr >> 16) & 0xFF) + ((addr >> 8) & 0xFF) +
               ((addr)&0xFF) + 4;
      ...
      if (type == '2') {
        ...
        /* copy data */
        if (pass == LAST_PASS) {
          for (i = 0; i < cnt; i++) putbyte(loccnt + i, buffer[i]);
        }
...
void do_rs(int *ip) {
  ...
  /* update 'rs' base */
  rsbase += value;
  if (rsbase & 0xFFFF0000) error("Address out of range!");
}
...
void do_ds(int *ip) {
  ...
  switch (section) {
    case S_ZP:
      /* zero page */
      if (addr > max_zp) max_zp = addr;
      break;

    case S_BSS:
      /* ram page */
      if (addr > max_bss) max_bss = addr;
      break;
  }
...
void do_fail(int *ip) { fatal_error("Compilation failed!"); }
...
void do_incchr(int *ip) {
  ...
  /* get args */
  if (!pcx_get_args(ip)) return;
  if (!pcx_parse_args(0, pcx_nb_args, &x, &y, &w, &h, 8)) return;
  ...
  if (lablptr) {
    ...
    if (lastlabl) {
      if (lastlabl->data_type == P_INCCHR) lastlabl->data_size += total;
    }
  }
...
void do_opt(int *ip) {
  ...
  for (;;) {
    ...
    /* set option */
    if (flag == '+') asm_opt[opt] = 1;
    if (flag == '-') asm_opt[opt] = 0;
  }
