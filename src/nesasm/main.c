...
int main(int argc, char **argv) {
  ...
  /* remove extension */
  if ((p = strrchr(prg_name, '.')) != NULL) *p = '\0';
    ...
    for (i = 1; i < argc; i++) {
      if (argv[i][0] == '-') {
        ...
        /* output level */
          ...
          /* check range */
          if (list_level < 0 || list_level > 3) list_level = 2;
        }
  ...
  for (pass = FIRST_PASS; pass <= LAST_PASS; pass++) {
    ...
    while (readline() != -1) {
      ...
      if (loccnt > 0x2000) {
        if (proc_ptr == NULL)
          ...
          sprintf(tmp, "Proc : '%s' is too large (code > 8KB)!",
                  proc_ptr->name);
          fatal_error(tmp);
        }
        break;
    ...
    /* adjust bank base */
    if (pass == FIRST_PASS) bank_base = calc_bank_base();
    ...
    if (pass == FIRST_PASS) {
      if (develo_opt || mx_opt || cd_opt || scd_opt) lablremap();
    }
  ...
  if (errcnt == 0) {
    ...
    /* save */
      ...
      /* s-record file */
      if (srec_opt) write_srec(out_fname, "s28", 0);
      ...
      /* binary file */
        ...
        /* write header */
        if (header_opt) machine->write_header(fp, max_bank + 1);
  ...
  /* close listing file */
  if (xlist && list_level) fclose(lst_fp);

  /* close input file */
  fclose(in_fp);

  /* dump the bank table */
  if (dump_seg) show_seg_usage();
...
void help(void) {
  /* check program name */
  if (strlen(prg_name) == 0) prg_name = machine->asm_name;
...
void show_seg_usage(void) {
  ...
  for (i = 0; i <= max_bank; i++) {
    ...
    /* scan */
    if (dump_seg == 1) continue;

    for (;;) {
      /* search section start */
      for (; addr < 8192; addr++)
        if (map[i][addr] != 0xFF) break;
      ...
      /* search section end */
      for (; addr < 8192; addr++)
        if ((map[i][addr] & 0x0F) != section) break;
