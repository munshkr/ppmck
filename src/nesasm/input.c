  ...
  if (expand_macro) {
    ...
    if (mlptr) {
      ...
      for (;;) {
        ...
        if (i >= LAST_CH_POS - 1) i = LAST_CH_POS - 1;
      }
      prlnbuf[i] = '\0';
      mlptr = mlptr->next;
      return (0);
...
int open_input(char *name) {
  ...
  if ((p = strrchr(temp, '.')) != NULL) {
    if (strchr(p, PATH_SEPARATOR)) strcat(temp, ".asm");
  } else {
    strcat(temp, ".asm");
  }
  ...
  /* open the file */
  if ((fp = open_file(temp, "r")) == NULL) return (-1);
