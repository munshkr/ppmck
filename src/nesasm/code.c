...
void class4(int *ip) {
  ...
  switch (mode) {
    case ACC:
      /* one byte */
      if (pass == LAST_PASS) putbyte(loccnt, opval);
  ...
  /* auto-increment */
  if (auto_inc) {
    if (pass == LAST_PASS) putbyte(loccnt, auto_inc);
...
void class7(int *ip) {
  ...
  /* make opcode */
  if (mode & (ZP | ZP_X)) opval = 0x83;
  if (mode & (ABS | ABS_X)) opval = 0x93;
  if (mode & (ZP_X | ABS_X)) opval += 0x20;
...
int getoperand(int *ip, int flag, int last_char) {
  ...
  switch (prlnbuf[*ip]) {
    ...
    default:
      ...
      if (code == 0x000000) mode &= (ABS | ZP | IMM);
      ...
      if (pass == LAST_PASS) {
        ...
        if (mode & (ZP | ZP_X | ZP_Y | ZP_IND | ZP_IND_X | ZP_IND_Y) & flag) {
          ...
          if ((value & 0xFFFFFF00) &&
              ((value & 0xFFFFFF00) != machine->ram_base))
        ...
        /* absolute modes */
          ...
          /* check address validity */
          if (value & 0xFFFF0000) error("Incorrect absolute address!");
        }
      }
      break;
  }
