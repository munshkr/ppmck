...
cont:
  ...
  while (!end) {
    ...
    /* number */
    if (isdigit(c)) {
      if (need_operator) goto error;
      if (!push_val(T_DECIMAL)) return (0);
    }

    /* symbol */
    else if (isalpha(c) || c == '_' || c == '.') {
      if (need_operator) goto error;
      if (!push_val(T_SYMBOL)) return (0);
    }
    ...
    /* operators */
      ...
      switch (c) {
        ...
        case '$':
          if (need_operator) goto error;
          if (!push_val(T_HEXA)) return (0);
          break;

        /* character prefix */
        case '\'':
          if (need_operator) goto error;
          if (!push_val(T_CHAR)) return (0);
          break;

        /* round brackets */
        case '(':
          if (need_operator) goto error;
          if (!push_op(OP_OPEN)) return (0);
          level++;
          expr++;
          break;
        case ')':
  ...
  if (undef) {
    if (pass == LAST_PASS) error("Undefined symbol in operand field!");
  }
...
int do_op(void) {
  ...
  /* second arg */
  if (op_pri[op] < 9) val[1] = val_stack[--val_idx];

  switch (op) {
    /* BANK */
    case OP_BANK:
      if (!check_func_args("BANK")) return (0);
      if (pass == LAST_PASS) {
    ...
    /* PAGE */
    case OP_PAGE:
      if (!check_func_args("PAGE")) return (0);
      val[0] = expr_lablptr->page;
      break;

    /* VRAM */
    case OP_VRAM:
      if (!check_func_args("VRAM")) return (0);
      if (pass == LAST_PASS) {
        if (expr_lablptr->vram == -1) error("No VRAM address for this symbol!");
      }
    ...
    /* PAL */
    case OP_PAL:
      if (!check_func_args("PAL")) return (0);
      if (pass == LAST_PASS) {
        if (expr_lablptr->pal == -1) error("No palette index for this symbol!");
      }
    ...
    /* DEFINED */
    case OP_DEFINED:
      if (!check_func_args("DEFINED")) return (0);
    ...
    /* SIZEOF */
    case OP_SIZEOF:
      if (!check_func_args("SIZEOF")) return (0);
      if (pass == LAST_PASS) {
